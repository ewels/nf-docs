...
class TestReproducibleOutput:
    ...
    def test_html_drops_the_generation_footer(self, sample_pipeline: Pipeline):
        output = HTMLRenderer(use_tailwind=False, include_generation_info=False).render(
            sample_pipeline
        )

        assert "Documentation generated by" not in output
        # The Nextflow attribution is static, so it stays
        assert "built with" in output
        assert "https://nextflow.io" in output

    def test_html_keeps_the_generation_footer_by_default(self, sample_pipeline: Pipeline):
