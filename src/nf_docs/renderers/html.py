...
from nf_docs.generation_info import NEXTFLOW_URL, get_generation_info, get_generation_timestamp
...
class HTMLRenderer(BaseRenderer):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            include_generation_info: Whether to show the "generated by nf-docs
                vX on <timestamp>" footer line, which is the only part of the
                page that changes between runs
        """
    ...
    def _render(
        ...
        *,
        ...
        # Get generation metadata - omitted entirely when the caller wants
        # reproducible output, since it carries a timestamp
        generation_info = get_generation_info() if self.include_generation_info else None
        generation_timestamp = get_generation_timestamp() if self.include_generation_info else None
    ...
    def render_pages(self, pipeline: Pipeline) -> dict[str, str]:
        """
        Render the pipeline as a single ``index.html`` page.

        Args:
            pipeline: The Pipeline model to render

        Returns:
            Mapping with one entry: ``index.html`` and its content
        """
        return {"index.html": self.render(pipeline)}
